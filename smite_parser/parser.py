...
class CombatLogParser:
    ...
    def _calculate_player_stats(self, session: Session, player_name: str) -> None:
        ...
        healing_done = 0
        cc_time_inflicted = 0
        damage_mitigated = 0  # Added for damage mitigation
        structure_damage = 0  # Added for structure damage
        ...
        for event in combat_events:
            ...
            if event.source_entity == player_name and event.event_type == "Damage":
                ...
                # Check if damage was to a structure
                if event.target_entity and any(struct in event.target_entity.lower() for struct in ["tower", "phoenix", "titan", "structure"]):
                    structure_damage += damage_amount
            
            # Player took damage and mitigated some
            if event.target_entity == player_name and event.event_type == "Damage":
                damage_taken += event.damage_amount or 0
                # If damage_mitigated is available in the event
                if hasattr(event, 'damage_mitigated') and event.damage_mitigated:
                    damage_mitigated += event.damage_mitigated
        ...
        stat.damage_mitigated = damage_mitigated  # Set damage mitigated value
        stat.structure_damage = structure_damage  # Set structure damage value
